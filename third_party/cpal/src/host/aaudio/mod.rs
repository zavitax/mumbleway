...
/// Which AAudio input preset new capture streams are opened with.
///
/// **The one thing changed in this vendored copy. See `PATCH.md`.**
///
/// AAudio's default is `VOICE_RECOGNITION`, and cpal never calls
/// `AAudioStreamBuilder_setInputPreset`, so that is what every capture stream
/// gets. On Android that has a consequence nothing in cpal's API can reach:
/// from Android 10 two apps may capture at once, the loser is handed digital
/// silence rather than an error, and only `VOICE_COMMUNICATION` and
/// `CAMCORDER` are treated as privacy sensitive — the presets that stop
/// another app capturing alongside. A voice app on the default preset can be
/// silenced by a navigation app listening for commands, with nothing in the
/// stream to say so.
///
/// Zero means "leave it alone", so an unset value is exactly upstream's
/// behaviour and this is inert until something asks. The whole thing is behind
/// the `android-input-preset` feature besides, because the call it makes is
/// absent from the NDK stubs below API 28 — see `PATCH.md`.
///
/// Read when a stream is built, so it must be set before the input device is
/// opened; changing it afterwards does nothing until the stream is reopened.
...
{
    #[allow(unused_mut)]
    let mut builder = configure_for_device(builder, device, config);
    // The vendored hunk, and the only one. See `ANDROID_INPUT_PRESET`.
    #[cfg(feature = "android-input-preset")]
    {
        let preset = ANDROID_INPUT_PRESET.load(std::sync::atomic::Ordering::Relaxed);
        if preset != 0 {
            builder = builder.input_preset(preset.into());
        }
    }
