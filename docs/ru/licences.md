...
клиент, работающий по протоколу Mumble. Название Mumble и товарные знаки
...
Протокол Mumble открыто задокументирован, и здесь он реализован независимо, по
этой документации. Сам Mumble распространяется под лицензией **BSD 3-Clause**, см.
...
| [DeepFilterNet](https://github.com/Rikorose/DeepFilterNet) | Очистка речи в начале тракта записи, вместе с весами модели. Низколатентная DFN3 идёт из крейта `deep_filter`, обычная DFN3 лежит в `core/models/` — для нижней ступени лестницы производительности | MIT / Apache-2.0 |
| [`tract`](https://github.com/sonos/tract) | Исполняет эту модель — на чистом Rust, без нативной среды, которую пришлось бы собирать под каждую платформу. `tract-core` взят в `third_party` и изменён одним фрагментом, без которого обычная DFN3 вообще не загружается, — см. `third_party/tract-core/PATCH.md` | MIT / Apache-2.0 |
...
| [`cpal`](https://crates.io/crates/cpal) | Кроссплатформенный доступ к аудиоустройствам. Взят в `third_party` и изменён одним фрагментом, чтобы поток записи на Android мог запросить телефонный входной пресет, который cpal иначе не запрашивает никогда — см. `third_party/cpal/PATCH.md` | Apache-2.0 |
| [`dasp_sample`](https://crates.io/crates/dasp_sample) | Преобразование форматов отсчётов | MIT / Apache-2.0 |
| [YAMNet](https://github.com/tensorflow/models/tree/master/research/audioset/yamnet) | Классификатор звука. **Больше не используется**: «Автоматически» теперь выбирает профиль по измеренному отношению сигнал/шум. По-прежнему входит в поставку как `assets/models/yamnet.tflite` — и потому остаётся в этом списке | Apache-2.0 |
| [LiteRT / TensorFlow Lite](https://ai.google.dev/edge/litert) | Запускает эту модель. На Android и iOS — из Maven и CocoaPods самой Google, на macOS — универсальная `libtensorflowlite_c` из состава `tflite_flutter`, на Windows — собрана из исходников TensorFlow r2.17 и лежит в `app/blobs/`, поскольку готовой сборки под неё никто не публикует | Apache-2.0 |
...
**Три из них поставляются, но не работают.** Классификатор звука убран из
тракта: «Автоматически» теперь выбирает профиль по отношению сигнал/шум,
измеренному в первую секунду речи, и никакая модель для этого не нужна. Сам
YAMNet, среда LiteRT и обвязка `tflite_flutter` всё ещё лежат внутри пакета,
потому что их изъятие затрагивает сборку на каждой платформе и проверить это
может только CI, — поэтому работа отложена, а не сделана.

Именно поэтому они перечислены выше. **На этой странице сказано, что входит в
поставку, а не что выполняется**: лицензионные обязательства следуют за копией
в пакете, и убрать строку только потому, что код перестал её вызывать, значило
бы преуменьшить то, что вам передают.
</div>

Всё остальное в тракте — эхоподавитель, гейт, экспандер, блок спектрального
вычитания, лимитер, АРУ, защита от обратной связи, детектор основного тона и
...
В начале того же README стоит **опровержение** этой оценки: измерения, на
которых она построена, были сделаны на звуке с собственного микрофона телефона,
а не с выносного микрофона гарнитуры, и два вывода из-за этого отозваны. Текст
оставлен на месте и помечен, а не удалён.

## Точность этой страницы

Лицензии перечислены по условиям, опубликованным самими проектами, и,
насколько известно, указаны верно — но эта страница остаётся сводкой, а не
