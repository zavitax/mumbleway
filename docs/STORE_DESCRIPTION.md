...
```
MumbleWay — клиент Mumble для разговора из-под мотоциклетного шлема на скорости.

Самое трудное — ветер. На скорости он громкий, широкополосный и не прекращается, и он сводит на нет шумоподавление обычных приложений для звонков: его настраивали под офисы. Тракт MumbleWay сделан под другую задачу: крутой фильтр верхних частот убирает ветер и гул мотора до того, как их увидит что-либо ещё, нейросетевой очиститель работает с широкополосным рёвом, а решение о передаче сравнивает голос с уровнем шума, который поднимается вместе со скоростью. Ровный гул поднимает порог вместе с собой и не переступает его, как бы громко ни звучал. Речь — переступает.

НУЖЕН СЕРВЕР MUMBLE
MumbleWay подключается к любому: к машине друга, к серверу в клубе, к публичному из встроенного каталога или к вашему собственному. Это клиент, а не сервис: ни учётной записи, ни подписки, ничьих чужих серверов посередине.
...
• Падающий сигнал, когда связь оборвалась, и восходящий, когда вернулась, — вы узнаёте это из гарнитуры, а не из тишины.
...
• Живой пинг по каждому серверу и то, идёт ли голос напрямую по UDP или через TCP, когда оператор не пропускает первое.
• Буфер дрожания проигрывает накопленное до двух раз быстрее, а не оставляет всех на секунду позади.
...
НА СТАРОМ ТЕЛЕФОНЕ
Блок звука приходит каждые 10 мс, и вся цепочка должна успеть до следующего. Если телефон не справляется, MumbleWay отключает ступени по одной, начиная с самых дешёвых, в измеренном порядке — и говорит, каких не стало, вместо того чтобы молча звучать хуже.
