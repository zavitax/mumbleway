...
файл часто до сих пор зовётся `mumble-server` или `murmurd`).

Понадобится что-то одно из этого:

- **Машина дома** — хватит запасного ПК, NAS или Raspberry Pi. Сервер Mumble
  для группы мотоциклистов почти не грузит процессор и занимает несколько
  мегабайт памяти.
- **Дешёвый VPS** — самый младший тариф у кого угодно более чем достаточен, и
  это избавляет от необходимости открывать порт дома.
...
Привычный дом для сервера Mumble и наименьшая морока.
...
Домашний Mac — вполне приличный сервер для группы, но он должен не засыпать:
...
| `users` | `20` | Ограничьте. Оставлять открытым нет причин. |
