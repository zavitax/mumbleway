...
class _SpectrumViewState extends State<SpectrumView> {
  ...
  /// The restoring bell, on its own notifier rather than through `_chain`.
  ///
  /// **`_chain` is deliberately not republished on every poll** — it is gated
  /// on [_signature], which lists the handful of fields the dots and the rows
  /// are drawn from, so that a status object the bridge rebuilds twenty times a
  /// second does not repaint all of them twenty times a second. The bell moves
  /// every block while somebody is speaking, which is exactly the traffic that
  /// gate exists to stop, and adding it to the signature would drag the dots,
  /// the profile line and the warnings along with it.
  ///
  /// This is the same arrangement `_gate` already uses for the gauges: its own
  /// notifier, a quantised value, and a real `==` so it stays silent when
  /// nothing a reader could see has moved.
  final _bell = ValueNotifier<_Bell?>(null);
  ...
  void initState() {
    ...
    _lifecycle = AppLifecycleListener(
      onStateChange: (s) => s == AppLifecycleState.resumed ? _start() : _stop(),
    );
  }
  ...
  void _onTick() {
    ...
    _gate.value = chain == null
        ...
        : GateSnapshot(
            ...
            applicable:
                !chain.warmingUp && chain.effectiveProfile != NoiseSetting.off,
          );

    // Null whenever nothing is being put back, which is most of the time:
    // the restoration runs on speech and nothing else.
    _bell.value = (chain == null || chain.restoreGainDb < 0.05)
        ? null
        : _Bell(
            centreHz: chain.restoreCentreHz,
            gainDb: chain.restoreGainDb,
            q: chain.restoreQ,
            maxDb: chain.restoreMaxDb,
          );
...
class _SpectrumBody extends StatelessWidget {
  ...
  /// The restoring bell, on its own listenable so the curve can follow the
  /// audio without dragging the rest of the panel with it.
  final ValueListenable<_Bell?> bell;
      ...
      builder: (context, isStalled, _) => ValueListenableBuilder<UiSpectrum?>(
        ...
        builder: (context, frame, _) => (frame == null || isStalled)
            ...
            : ValueListenableBuilder<UiChainStatus?>(
                ...
                builder: (context, status, _) => ValueListenableBuilder<_Bell?>(
                  valueListenable: bell,
                  builder: (context, bellNow, _) => CustomPaint(
                    size: Size.infinite,
                    painter: _SpectrumPainter(
                      ...
                      // From its own notifier, not from `status`. The
                      // chain's notifier is signature-gated and the bell is not
                      // in the signature, so reading it there left the curve at
                      // whatever it held when some unrelated field last moved —
                      // which in practice was never, so it never drew at all.
                      restore: bellNow,
...
class _SpectrumPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    mark(
      opensAtDb,
      StatusColors.connecting.withValues(alpha: 0.7),
      opensAtLabel,
    );
  ...
  ///
  /// **Zero gain at the bottom, the cap at the top.** It was first drawn on the
  /// plot's own decibel ruler, so that its height above the floor line *was*
  /// its gain — tidier to describe and unreadable in practice. The axis spans
  /// ninety-odd decibels and the bell spans twelve, so the whole curve lived in
  /// the bottom eighth of the display and read as a flat line wherever it was
  /// not at full lift, which is most of the time.
  ///
  /// A scale nobody can see is not a more honest scale. This one costs a reader
  /// knowing that the top edge is the cap, and in exchange the shape becomes
  /// the thing on screen that moves most — which is the whole reason to draw
  /// it.
  ///
  ...
  void _paintBell(
    ...
    final bell = restore;
    if (bell == null || bands < 2 || bell.maxDb <= 0) return;
    final a = math.pow(10, bell.gainDb / 40).toDouble();

    // Gain in dB to a height on this curve's own scale.
    double yOfGain(double g) =>
        size.height - (g / bell.maxDb).clamp(0.0, 1.0) * size.height;
...
class _Bell {
  _Bell({
    ...
    required this.q,
    required this.maxDb,
  }) : centreHz = (centreHz / 5).roundToDouble() * 5,
       gainDb = (gainDb * 10).roundToDouble() / 10;

  /// Quantised on the way in: 5 Hz and a tenth of a decibel, both well under
  /// what a curve a hundred pixels wide can show. Without it this notifier
  /// would fire on every poll for a curve that had not visibly moved, which is
  /// the cost the whole panel is arranged to avoid.
  final double centreHz, gainDb, q;

  /// The cap, which is the top of this curve's own scale. Carried rather than
  /// assumed for the same reason `q` is.
  final double maxDb;
  ...
  @override
  int get hashCode => Object.hash(centreHz, gainDb, q, maxDb);
}
