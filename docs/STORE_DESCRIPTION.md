...
• Голосовая активация открывает канал на 240 мс раньше, чем принимает решение, — у слова остаётся первый согласный. Потом задержка сходит к 60 мс.
...
В тишине профиль «Лёгкий» почти ничего не убирает, а диагностика показывает, что каждая ступень делает с голосом, — настройка глазами, а не наугад. За столом и на скорости 120 км/ч это один и тот же клиент: Opus 48 кГц, без учётной записи и телеметрии.
